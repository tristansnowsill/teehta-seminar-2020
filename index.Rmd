...
<table class="academic-table">
  <thead>
    ...
    <tr>
      <td colspan="2">Cohort/aggregate levels/counts</td>
      <td colspan="2">Individual level</td>
    </tr>
    <tr>
      <td>Expected value, continuous state, deterministic</td>
      <td>Markovian, discrete state, stochastic</td>
      <td>Markovian, discrete state, individuals</td>
      <td>Non-Markovian, discrete-state, individuals</td>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>1</td>
      <td rowspan="2">No interaction allowed</td>
      <td>Untimed</td>
      <td>Decision tree rollback</td>
      <td>Simulated decision tree</td>
      <td colspan="2">Individual sampling model (ISM): Simulated patient-level decision tree (SPLDT)</td>
    </tr>
    <tr>
      <td>2</td>
      <td>Timed</td>
      <td>Markov model (evaluated deterministically)</td>
      <td>Simulated Markov model (SMM)</td>
      <td colspan="2">Individual sampling model (ISM): Simulated patient-level Markov model (SPLMM) (variations as in quadrant below for patient level models with interaction)</td>
    </tr>
    <tr>
      <td>3</td>
      <td rowspan="2">Interaction allowed</td>
      <td>Discrete time</td>
      <td>System dynamics (finite difference equations, FDE)</td>
      <td>Discrete time Markov chain model (DTMC)</td>
      <td>Discrete-time individual event history model (DT, IEH)</td>
      <td>Discrete individual simulation (DT, DES)</td>
    </tr>
    <tr>
      <td>4</td>
      <td>Continuous time</td>
      <td>System dynamics (ordinary differential equations, ODE)</td>
      <td>Continuous time Markov chain model (CTMC)</td>
      <td>Continuous time individual event history model (CT, IEH)</td>
      <td>Discrete event simulation (CT, DES)</td>
    </tr>
  </tbody>
</table>
')
...
<p class="footnote">Source: Brennan A, et al. A taxonomy of model structures for economic evaluation of health technologies. <em>Health Econ</em> 2006; <strong>15</strong>: 1295&ndash;1310</p>
...
```{r markov-cohort-simulation-diagram, echo=FALSE}
p <- DiagrammeR::grViz('
digraph G {
  subgraph sg1 {
    rank = same;
...
htmlwidgets::saveWidget(p, "fig01.html")
```
...
```{r markov-cohort-simulation-tunnels, echo=FALSE}
p <- DiagrammeR::grViz('
digraph G {
  subgraph sg1 {
    rank = same;
    Healthy;
    Diseased1 [label="Diseased (1)"];
    Diseased2 [label="Diseased (2)"];
    Diseased3 [label="Diseased (3)"];
    Diseased4 [label="Diseased (4+)"];
  }
  Healthy -> Diseased1 -> Diseased2 -> Diseased3 -> Diseased4;
  {Diseased1, Diseased2, Diseased3, Diseased4} -> Dead;
  Healthy -> Dead
  Healthy -> Healthy;
  Diseased4 -> Diseased4;
  Dead -> Dead;
...
htmlwidgets::saveWidget(p, "fig02.html")
```
...
digraph G {
  ...
  Healthy -> Diseased [label="T&#x2081;"];
  Diseased -> Dead [label="T&#x2083;"];
  Healthy -> Dead [label="T&#x2082;"];
}

[1]: expression(T_1)
', width = 720, height = 300)
htmlwidgets::saveWidget(p, "fig03.html")
```
...
\[
  (f_X \ast f_Y)(t) = \int_{0}^{\infty}{f_X(s)f_Y(t-s)\mathrm{d}s}
\]
...
```{r fft-d3, warning = FALSE}
dt <- tibble(i = 1:64,
             t = seq(0, 10 * 63 / 31, length.out = 64),
             f1 = c(dexp(t[1:32]), rep(0, 32)),
             fft_re = Re(fft(f1)),
             fft_im = Im(fft(f1)),
             fft_mod = Mod(fft(f1)),
             fft_arg = Arg(fft(f1)),
             f2 = c(dlnorm(t[1:32], meanlog = 1), rep(0, 32)),
             fft2_mod = Mod(fft(f2)),
             fft2_arg = Arg(fft(f2)),
             fft12_mod = Mod(fft(f1) * fft(f2)),
             fft12_arg = Arg(fft(f1) * fft(f2)),
             f12 = 2*10*Re(fft(fft(f1) * fft(f2), inverse = TRUE)) / (4 * 32^2),
             type = rep(c("Model", "Zero-padding"), each = 32))
viz <- r2d3("assets/js/fft-example.js",
            data = dt,
            dependencies = "d3-jetpack")
save_d3_html(viz, file = "fft-d3.html")
```
...
```{r fft-example-fig, fig.width = 6, fig.height = 3, dpi = 300, out.width = 900}
ggplot(pivot_longer(sim_FFT, 2:4), aes(x = t, y = value, colour = name)) +
  geom_point() +
  geom_line() +
  labs(x = "Time", y = "Probability density function") +
  scale_colour_discrete("PDF", labels = c(f1 = expression(T[1]), f2 = expression(T[2]), f12 = expression(T[1] + T[2])))
```
...
```{r ce-scatter-plot, fig.width = 6, fig.height = 3, dpi = 300, out.width = 900}
ggplot(tibble(qaly_inc = rnorm(1000, 0.4, 0.2), cost_inc = rnorm(1000, 1600 * qaly_inc + 8000, 2000)),
       aes(x = qaly_inc, y = cost_inc)) +
  geom_point(alpha = 0.5) +
  geom_point(data = ~summarise(.data = ., across(.fns = mean)), size = 3, colour = "red") +
  labs(x = "Incremental QALYs", y = "Incremental costs", title = "Cost-effectiveness scatter plot") +
...
```{r ceac-plot, fig.width = 6, fig.height = 3, dpi = 300, out.width = 900}
ggplot(tibble(icer = rgamma(200, shape = 6, scale = 6000)),
       aes(x = icer)) +
  stat_ecdf() +
  labs(x = "Cost-effectiveness threshold",
...
```{r nmb-plot, fig.width = 6, fig.height = 3, dpi = 300, out.width = 900}
tibble(qaly_inc = rnorm(1000, 0.4, 0.4),
       cost_inc = rnorm(1000, 1600 * qaly_inc + 8000, 10000),
       dummy = 1) %>%
  full_join(tibble(wtp = seq(0, 100000, by = 1000), dummy = 1)) %>%
  mutate(nmb = wtp * qaly_inc - cost_inc) %>%
  group_by(wtp) %>%
  summarise(nmb_mean = mean(nmb), nmb_ci = list(t.test(nmb)$conf.int)) %>%
  unnest_wider(nmb_ci) %>%
  rename(nmb_lcl = ...1, nmb_ucl = ...2) %>%
ggplot(aes(x = wtp, y = nmb_mean, ymin = nmb_lcl, ymax = nmb_ucl)) +
...
```{r evpi-plot, fig.width = 6, fig.height = 3, dpi = 300, out.width = 900}
tibble(qaly_inc = rnorm(1000, 0.4, 0.4),
       cost_inc = rnorm(1000, 1600 * qaly_inc + 8000, 10000),
       dummy = 1) %>%
  full_join(tibble(wtp = seq(0, 100000, by = 500), dummy = 1)) %>%
  mutate(inmb = wtp * qaly_inc - cost_inc,
         abs_inmb = abs(inmb)) %>%
  group_by(wtp) %>%
  summarise(evpi = mean(abs_inmb) - abs(mean(inmb))) %>%
ggplot(aes(x = wtp, y = evpi)) +
  geom_line() +
  labs(x = "Willingness-to-pay",
...
```{r delta-ceac, fig.width=6, fig.height=3, dpi=300, out.width=900}
ggplot(tibble(wtp = seq(0, 100000, by = 500),
              m = -8640 + wtp*0.4,
              s = sqrt(wtp^2 * 0.4^2 - 2*wtp*256 + (10000^2 + 640^2)),
              ceac = pnorm(m, sd = s)),
       aes(x = wtp, y = ceac)) +
  geom_line() +
  labs(x = "Cost-effectiveness threshold",
...
```{r delta-evpi, fig.width=6, fig.height=3, dpi=300, out.width=900}
ggplot(tibble(wtp = seq(0, 100000, by = 500),
              m = -8640 + wtp*0.4,
              s = sqrt(wtp^2 * 0.4^2 - 2*wtp*256 + (10000^2 + 640^2)),
              evpi = (s*sqrt(2/pi)*exp(-(m^2)/(2*s^2)) - m*(1 - 2*pnorm(m/s))) - abs(m)),
       aes(x = wtp, y = evpi)) +
  geom_line() +
  labs(x = "Willingness-to-pay",
       y = "Expected value of perfect information",
...
\frac{\partial\mathbf{x}_t}{\partial\theta_i} &= \begin{bmatrix}
  \left(\frac{\partial P_{t-1}^\top}{\partial\theta_1} \mathbf{x}_{t-1} + P_{t-1}^\top \frac{\partial\mathbf{x}_{t-1}}{\partial\theta_1}\right) &
  \cdots &
  \left(\frac{\partial P_t^\top}{\partial\theta_n} \mathbf{x}_t + P_t^\top \frac{\partial\mathbf{x}_t}{\partial\theta_n}\right)
\end{bmatrix}
\end{aligned}
\]
